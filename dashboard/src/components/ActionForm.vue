...
const props = defineProps({ action: { type: Object, required: true }, rule: { type: Object, required: true }, index: { type: Number, required: true } })
...
const params = computed(() => getVisibleParamDefs(meta.value, ensureParams(props.action)))
const sources = computed(() => buildBindingSources(props.rule, props.index, store.schema))
const outputHint = computed(() => outputDefs(meta.value).map(output => `${props.action.binding_id} · ${output.label || output.name}`).join('　'))
const failureSources = index => buildFailureBindingSources(props.rule, props.index, index, store.schema)
</script>
