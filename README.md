...
内置插件现有 18 个触发器、28 个动作。

触发器按用途：

- 时间：定时、计划任务、开机启动、手动触发。
- 输入与状态：全局热键、空闲检测、锁屏/解锁、系统资源占用。
- 电源与设备：电源状态、电量阈值、音频设备、USB 设备。
- 网络：网络连通状态、WiFi 网络变化。
- 软件状态：进程启动/退出、窗口标题变化。
- 内容变化：剪贴板变化、文件夹变化。
...
- 实验性 AI 功能只生成待检查的规则候选或插件元数据提案，不直接保存、安装或运行。

完整插件清单见 Dashboard 的插件页；内置源码在 `notmyfault/triggers/` 和 `notmyfault/actions/`，随包提供但需要手动安装的可选插件在 `notmyfault/bundled/`。
