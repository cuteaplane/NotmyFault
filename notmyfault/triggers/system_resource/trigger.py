...
def _get_usage(resource: str, disk_path=None) -> float:
    if resource == "cpu":
        return psutil.cpu_percent(interval=0)
    elif resource == "memory":
        return psutil.virtual_memory().percent
    elif resource == "disk":
        return psutil.disk_usage(disk_path or (os.environ.get("SystemDrive", "C:") + os.sep if os.name == "nt" else "/")).percent
...
class SystemResourceTrigger(PollingTrigger):
    ...
    def validate(self):
        self.disk_path = str(self.config.get("disk_path", "")).strip() or None
    ...
    def poll(self):
        if self.resource == "network":
            ...
            value = _get_usage(self.resource, self.disk_path) if self.resource == "disk" else _get_usage(self.resource)
