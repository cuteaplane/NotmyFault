...
def prepare_rules(rules, triggers_meta, actions_meta, *, validate=True):
    normalized = normalize_rule_input(rules) if validate else normalize_rules(rules)
    if validate:
        valid, total, issues, _warnings = validate_rules(normalized, triggers_meta, actions_meta)
        if valid != total:
            raise ValueError("；".join(f"{name}: {message}" for name, message in issues))
