...
import notmyfault.actions.uia_automation.action as macro_action
import notmyfault.actions.uia_automation.component as macro_component
...
class TestExecuteMacro:
    ...
    def test_run_rejects_missing_macro(self):
        with pytest.raises(ValueError):
            macro_action.run_with_context(None, {"mode": "macro"}, {})
    ...
    def test_run_accepts_owned_macro(self, monkeypatch):
        ...
        value = make_owned_value(
            "io.github.notmyfault.uia_automation",
            "mouse_macro",
            1,
            {"version": 1, "steps": STEPS},
            "1 个操作宏 · 2 步",
        )
        assert macro_action.run_with_context(
            None,
            {"mode": "macro", "macro": value},
            {},
        ) == {
            "executed": 2
        }
    ...
    def test_run_rejects_other_plugin_data(self):
        ...
        with pytest.raises(ValueError, match="归属不匹配"):
            macro_action.run_with_context(
                None,
                {"mode": "macro", "macro": value},
                {},
            )

    def test_control_mode_dispatches_with_cancellation(self, monkeypatch):
        cancellation = object()
        calls = []
        monkeypatch.setattr(
            macro_action,
    ...
    @pytest.mark.parametrize(
        ("mode", "function_name", "expected"),
        [
            ("focus_window", "focus_selector_window", {"focused": True}),
            ("read_text", "read_selector_text", {"text": "示例"}),
        ],
    )
    def test_target_modes_dispatch_with_cancellation(
        self, monkeypatch, mode, function_name, expected
    ):
        cancellation = object()
        calls = []
        monkeypatch.setattr(
    ...
    def test_wait_mode_dispatches_timeout_and_cancellation(self, monkeypatch):
        cancellation = object()
        calls = []
        monkeypatch.setattr(
            macro_action,
        ...
        result = macro_action.run_with_context(
            None,
            {"mode": "wait", "target": SELECTOR, "wait_seconds": 45},
            {"runtime": {"cancellation": cancellation}},
        )
...
class TestMacroComponent:
    ...
    def make_context(current_value=None):
        ...
        session = ExtensionSession(
            ...
            data_type=registry.data_type("uia_automation", "mouse_macro"),
    ...
    def test_commit_builds_macro_value(self):
        ...
        assert result["value"]["$type"] == (
            "io.github.notmyfault.uia_automation/mouse_macro@1"
        )
